...
export default function Module3() {
    ...
    return (
        <main className="container">
            ...
            {/* ═══════════════════════════════════════════════ */}
            {/* PHASE 4: SIMULATE — Animated 2D Scene          */}
            {/* ═══════════════════════════════════════════════ */}
            {phase === 'SIMULATE' && (
                <div className="panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease' }}>
                    ...
                    <div style={{ display: 'flex', height: '75vh' }}>
                        ...
                        <div style={{ 
                            ...
                            {/* Sky gradient — changes per scene */}
                            <div style={{ 
                            ...
                            {(currentSceneKey === 'open_gates' || currentSceneKey === 'success') ? (
                                <div style={{ position: 'absolute', top: '40px', right: '60px', width: '60px', height: '60px', borderRadius: '50%', background: '#fef3c7', boxShadow: '0 0 40px rgba(254,243,199,0.6)', zIndex: 2 }}></div>
                            ) : currentSceneKey !== 'failed' ? (
                                <div style={{ position: 'absolute', top: '30px', right: '80px', width: '70px', height: '70px', borderRadius: '50%', background: '#fde047', boxShadow: '0 0 60px rgba(253,224,71,0.5)', zIndex: 2 }}></div>
                            ) : null}
                            ...
                            {(currentSceneKey === 'open_gates' || currentSceneKey === 'success') && (
                                <>
                                    {[...Array(15)].map((_, i) => (
                                        <div key={`star-${i}`} style={{ position: 'absolute', top: `${5 + Math.random() * 35}%`, left: `${Math.random() * 100}%`, width: '3px', height: '3px', borderRadius: '50%', background: 'white', animation: `starTwinkle ${1 + Math.random() * 2}s ease-in-out infinite`, animationDelay: `${Math.random() * 2}s`, zIndex: 2 }}></div>
                                    ))}
                                </>
                            )}
                            ...
                            {currentSceneKey !== 'failed' && (
                                <>
                                    <svg style={{ position: 'absolute', top: '8%', zIndex: 3, animation: 'cloudDrift 25s linear infinite', opacity: 0.7 }} width="120" height="50" viewBox="0 0 120 50">
                                        <ellipse cx="60" cy="35" rx="55" ry="15" fill="white" opacity="0.8"/>
                                        <ellipse cx="40" cy="25" rx="30" ry="18" fill="white" opacity="0.9"/>
                                        <ellipse cx="75" cy="22" rx="25" ry="16" fill="white"/>
                                    </svg>
                                    <svg style={{ position: 'absolute', top: '18%', zIndex: 3, animation: 'cloudDrift 35s linear infinite', animationDelay: '-12s', opacity: 0.5 }} width="90" height="40" viewBox="0 0 90 40">
                                        <ellipse cx="45" cy="28" rx="40" ry="12" fill="white" opacity="0.8"/>
                                        <ellipse cx="30" cy="20" rx="22" ry="14" fill="white"/>
                                        <ellipse cx="55" cy="18" rx="18" ry="12" fill="white"/>
                                    </svg>
                                </>
                            )}

                            {/* Ground */}
                            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '35%', background: currentSceneKey === 'failed' ? '#451a03' : 'linear-gradient(180deg, #16a34a, #15803d)', zIndex: 4, transition: 'all 0.8s' }}>
                                {/* Grass blades */}
                                <svg style={{ position: 'absolute', top: '-8px', left: 0, width: '100%', height: '20px' }} viewBox="0 0 1000 20" preserveAspectRatio="none">
                                ...
                                {/* Path line */}
                                <div style={{ position: 'absolute', bottom: '30%', left: 0, right: 0, height: '6px', background: 'rgba(0,0,0,0.15)', borderRadius: '3px' }}></div>
                            </div>

                            {/* ── SVG CHARACTER: Student ────────── */}
                            <div style={{ 
                                position: 'absolute', bottom: '33%', left: '12%', zIndex: 10,
                            ...
                            <div key={currentSceneKey} style={{ animation: 'sceneTransition 0.6s ease' }}>
                                ...
                                {currentSceneKey === 'book_venue' && (
                                    <div style={{ position: 'absolute', right: '10%', bottom: '28%', zIndex: 8, animation: 'propSlideIn 0.8s ease' }}>
                                        ...
                                        {/* Clipboard */}
                                        <svg style={{ position: 'absolute', left: '-50px', bottom: '20px', animation: 'propBounceIn 0.5s ease 0.4s both' }} width="50" height="60" viewBox="0 0 50 60">
                                ...
                                {currentSceneKey === 'hire_vols' && (
                                    <div style={{ position: 'absolute', right: '8%', bottom: '33%', zIndex: 8, display: 'flex', gap: '8px', animation: 'propBounceIn 0.6s ease' }}>
                                        {['#ef4444', '#3b82f6', '#10b981', '#f59e0b'].map((color, i) => (
                                            <svg key={i} width="40" height="65" viewBox="0 0 40 65" style={{ animation: `crowdWave 1.2s ease-in-out infinite`, animationDelay: `${i * 0.2}s` }}>
                                        ...
                                        {/* Walkie-talkie icon */}
                                        <svg style={{ position: 'absolute', top: '-20px', right: '-10px', animation: 'propBounceIn 0.5s ease 0.5s both' }} width="30" height="40" viewBox="0 0 30 40">
                                ...
                                {currentSceneKey === 'order_food' && (
                                    <div style={{ position: 'absolute', right: '8%', bottom: '30%', zIndex: 8, animation: 'propSlideIn 0.8s ease' }}>
                                ...
                                {currentSceneKey === 'marketing' && (
                                    <div style={{ position: 'absolute', right: '10%', bottom: '35%', zIndex: 8 }}>
                                        {/* Banner */}
                                        <svg style={{ animation: 'bannerDrop 0.8s ease' }} width="200" height="120" viewBox="0 0 200 120">
                                        ...
                                        {/* Megaphone */}
                                        <svg style={{ position: 'absolute', left: '-60px', top: '20px', animation: 'propBounceIn 0.5s ease 0.5s both' }} width="60" height="50" viewBox="0 0 60 50">
                                ...
                                {currentSceneKey === 'open_gates' && (
                                    <div style={{ position: 'absolute', right: '5%', bottom: '28%', zIndex: 8, animation: 'propSlideIn 1s ease' }}>
                                        {/* Gate arch */}
                                        <svg width="180" height="130" viewBox="0 0 180 130">
                                            ...
                                            {[30, 60, 90, 120, 150].map((x, i) => (
                                                <circle key={i} cx={x} cy="12" r="4" fill={['#ef4444', '#fbbf24', '#10b981', '#3b82f6', '#ef4444'][i]} style={{ animation: `starTwinkle ${0.5 + i * 0.2}s ease infinite` }}/>
                                            ))}
                                        </svg>
                                        {/* Crowd streaming in */}
                                        <div style={{ display: 'flex', gap: '4px', marginTop: '-20px', justifyContent: 'center' }}>
                                            {['#ef4444', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899'].map((c, i) => (
                                                <svg key={i} width="24" height="40" viewBox="0 0 24 40" style={{ animation: `crowdWave 0.8s ease infinite`, animationDelay: `${i * 0.15}s` }}>
                                ...
                                {currentSceneKey === 'success' && (
                                    <>
                                        <div style={{ position: 'absolute', top: '15%', left: '50%', transform: 'translateX(-50)', zIndex: 12, textAlign: 'center', animation: 'propBounceIn 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275)' }}>
                                            <div style={{ fontSize: '80px', filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.4))' }}>🏆</div>
                                        </div>
                                        {/* Confetti pieces */}
                                        {[...Array(20)].map((_, i) => (
                                            <div key={`c-${i}`} style={{ position: 'absolute', top: `${10 + Math.random() * 30}%`, left: `${5 + Math.random() * 90}%`, width: '8px', height: '8px', borderRadius: i % 2 === 0 ? '50%' : '2px', background: ['#ef4444', '#fbbf24', '#3b82f6', '#10b981', '#a855f7', '#ec4899'][i % 6], animation: `confetti ${1.5 + Math.random() * 2}s ease-out infinite`, animationDelay: `${Math.random() * 1.5}s`, zIndex: 15 }}></div>
                                        ))}
                                    </>
                                )}

                                {/* FAILED: Crack overlay */}
                                {currentSceneKey === 'failed' && (
                                    <div style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translateX(-50%)', zIndex: 12, textAlign: 'center', animation: 'propBounceIn 0.5s ease' }}>
                                        <div style={{ fontSize: '80px' }}>💥</div>
                                        <div style={{ color: '#fca5a5', fontSize: '24px', fontWeight: '800', marginTop: '10px', textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>ALGORITHM CRASHED</div>
                                    </div>
                                )}
                            </div>

                            {/* ── Speech Bubble (always shows current action) ── */}
                            {simStatus === 'running' && simStep < sequence.length && (
                                <div key={`bubble-${simStep}`} style={{ 
                                    position: 'absolute', bottom: '40%', left: '25%', zIndex: 20,
                                    background: 'white', borderRadius: '20px', padding: '14px 22px',
                                    boxShadow: '0 8px 20px rgba(0,0,0,0.2)', maxWidth: '280px',
                                    animation: 'speechBubble 0.4s ease',
                                    fontSize: '16px', color: '#1e293b', fontWeight: '600'
                                }}>
                                    {scene.subtitle}
                                    {/* Bubble tail */}
                                    <div style={{ position: 'absolute', bottom: '-8px', left: '20px', width: '16px', height: '16px', background: 'white', transform: 'rotate(45deg)', borderRadius: '2px' }}></div>
                                </div>
                            )}

                            {/* Status badge */}
                            <div style={{ position: 'absolute', top: '16px', left: '16px', display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)', padding: '8px 16px', borderRadius: '20px', zIndex: 20, border: '1px solid rgba(255,255,255,0.15)' }}>
                                <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: simStatus === 'running' ? '#ef4444' : (simStatus === 'success' ? '#10b981' : '#f59e0b'), animation: simStatus === 'running' ? 'pulse 1s infinite' : 'none' }}></div>
                                <span style={{ color: 'white', fontSize: '13px', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase' }}>
                            ...
                            <div style={{ position: 'absolute', bottom: '12px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px', zIndex: 20 }}>
                                {sequence.map((_, i) => (
                                    <div key={i} style={{ width: i === simStep && simStatus === 'running' ? '24px' : '10px', height: '10px', borderRadius: '5px', background: i < simStep ? '#10b981' : (i === simStep && simStatus === 'running' ? '#fbbf24' : 'rgba(255,255,255,0.25)'), transition: 'all 0.4s ease' }}></div>
                                ))}
                            </div>
                        </div>
